node {
    ...
    docker.image('mariadb:10.3').withRun('-e "MYSQL_ALLOW_EMPTY_PASSWORD=1"') { c ->
        ...
        docker.image('selenium/standalone-chrome-debug:3.7.1-beryllium').inside("--link ${c.id}:webdriver") {
            /* We should wait, but it's not simple and Drupal has to install anyways. */
        }
        docker.image('andrewberry/drupal_tests:0.2.0').inside("--link ${c.id}:db") {
        sh '''#!/bin/bash
